...
public class DateUtils {
	 ...
	 * @return
	 */
	public static boolean notExpiresIn(Date date, int second){
		Date currentDate = new Date();
		return (currentDate.getTime() - date.getTime()) < second * 1000;
	}
	
	public static boolean expiresIn(Date date, int second){
		return !notExpiresIn(date, second);
	}
    ...
    public static Date secondToDate(String second){
        ...
        return new java.util.Date(Long.parseLong(second)*1000);
    }
    ...
    /**
     * 时分秒格式数据 转换为秒
     * @param durationTime （00:00:00）
     * @return
     */
    public static Integer timeToLong(String durationTime){
    	Integer lon = 0; 
    	try {
    		String[] ss = durationTime.split(":");
