var setting = {
	async : {
		enable : true,
		url : contextpath+"/courseCategory/getCourseCategoryJson",
		autoParam : [ "id=parent" ],
...
function updateDataTabs(event, treeId, treeNode, clickFlag) {
	...
	$("#courselistdatagrid").datagrid('load', {
		categoryCode : treeNode.code // 左侧课程分类id
	});
}
...
function add_course(){
	...
	$('#t_course_categoryId').val($("#cc_ztree_node_id").val());
	$('#t_course_categoryName').textbox('setValue', $("#cc_ztree_node_name").val());
	$('#t_course_categoryCode').val($("#cc_ztree_node_code").val());
	$('#t_course_categoryId').val($("#cc_ztree_node_id").val());
	//$("#t_category_status").attr("checked",true);
}
