...
(function () {
    ...
    Editor.prototype = /**@lends baidu.editor.Editor.prototype*/{
        ...
        _initEvents : function () {
            ...
            if(browser.ie){
                domUtils.on( doc, 'mousedown', function(e){
                    if(e.button == 2){
                        alert("请使用ctrl+v进行粘贴！");
                    }
                });
...
//                img.style.cssText += ';margin:' + (first.margin||0) +"px;" + 'border:' + (first.border||0) +"px solid #000";
            ...
            ci = opt[0];
            if(opt.length == 1){
                str = '<p><img src="'+ci.src+'" '+ (ci.data_ue_src ? ' data_ue_src="' + ci.data_ue_src +'" ':'') +
                        (ci.width ? 'width="'+ci.width+'" ':'') +
                        (ci.height ? ' height="'+ci.height+'" ':'') +
                        (ci['floatStyle']&&ci['floatStyle']!='center' ? ' style="float:'+ci['floatStyle']+';"':'') +
                        (ci.title?' title="'+ci.title+'"':'') + ' border="'+ (ci.border||0) + '" hspace = "'+(ci.hspace||0)+'" vspace = "'+(ci.vspace||0)+'" /></p>';
                ...
                if ( current.nodeType == 3 || !block( current ) ) {
                    ...
                    if (para.hasChildNodes()) {
                        ...
                        for (var i=0; i<para.childNodes.length; i++) {
                            if(para.childNodes[i].tagName == 'BR')
                                {
...
UE.commands['indent'] = {
    execCommand : function() {
         var me = this;
         if(me.body.childNodes.length != 1){
            $(me.body.childNodes).each(function(){
                if($(this).html() != null) {$(this).html($(this).html().replace(/&nbsp;/ig,''));}
                $(this).find('br').remove();
                if($(this).html() == '<br>' || $(this).html() == ''){$(this).remove();}
                if(this.tagName && this.tagName.toLowerCase() != 'h3'){
                    $(this).append('<br /><br />');
                }
                var range = me.selection.getRange();
                range.setCursor();
    ...
    UE.plugins['paste'] = function() {
        ...
        function filter(div){
            ...
            if ( div.firstChild ) {
                    ...
                    //html = div.innerHTML;
                    html = $.htmlClean(div.innerHTML, {allowedTags:['p', 'a']});
        ...
        me.addListener('ready',function(){
            ...
            if(browser.opera){
                if(baidu.editor.browser.mac){
                    var kc = []
                    domUtils.on(me.body, 'keydown', function(e){
                        if(e.keyCode == 17){
                            alert('暂不支持此浏览器！');
                        }
                    })
...
(function (){
    ...
    var btnCmds = [
        ...
        'SelectAll', 'Print', 'Link', 'InsertVideo'];
    k = btnCmds.length;
    while (k --) {
        cmd = btnCmds[k];
            ...
            return function (editor, title){
                ...
                var ui = new editorui.Button({
                    ...
                    onclick: function (){
                        ...
                        if(cmd == 'Print'){
                            $('#ueditor_image_popup' + base_id).show();
                            $('#ueditor_image_popup' + base_id + ' div.edui-popup-all-content').draggable({
                                handle: '#ueditor_image_popup_top' + base_id
                                , scroll: false
                            });
                         }else if(cmd == 'InsertVideo'){                            
                            $('#ueditor_video_popup' + base_id).show();                            
                            $('#ueditor_video_popup' + base_id + ' input#ueditor_video_url' + base_id).focus();
                            $('#ueditor_video_popup' + base_id + ' div.edui-popup-all-content').draggable({
                                handle: '#ueditor_video_popup_top' + base_id
                                , scroll: false
                            });
                         }else if(cmd == 'Link'){
                            $('#ueditor_link_popup' + base_id).show();
                            $('#ueditor_link_popup' + base_id + ' input#ueditor_link_href' + base_id).focus();
                            $('#ueditor_link_popup' + base_id + ' div.edui-popup-all-content').draggable({
                                handle: '#ueditor_link_popup_top' + base_id
                                , scroll: false
                            });
                         }else if(cmd =='Bold'){
                            var me = editor,
                            ...
                            if (node && node.tagName.toLowerCase() == 'h3') {
                                range = me.selection.getRange();
                                bk = range.createBookmark();
                                if (range.collapsed) {
                                    ...
                                    node = domUtils.findParent(common, function(node) {
                                        return node.tagName.toLowerCase() == 'h3'
                                    }, true);
                                    if (node) {
                                ...
                                if (dtd.$block[tagName]) {
                                    me.execCommand('paragraph', tagName, tmpObj,'customstyle');
                                    $("#" + editor.ui.toolbars[0].items[0].id).addClass("edui-toolbar-menu-checked")
                                    range = me.selection.getRange();
                                    if (!range.collapsed) {
                                        range.collapse();
                                        node = domUtils.findParent(me.selection.getStart(), function(node) {
    ...
    me.addListener('keyup', function(type, evt) {
        ...
        if (keyCode == 32 || keyCode == 13) {
            ...
            if (range.collapsed) {
                ...
                if (node && node.tagName.toLowerCase() == 'br' && domUtils.isEmptyNode(node)) {

                    var p = me.document.createElement('p');
                    domUtils.insertAfter(node.parentNode, p);
                    domUtils.fillNode(me.document, p);
                    domUtils.remove(node.parentNode);
    ...
    EditorUI.prototype = {
        ...
        initEditorUI: function () {
            ...
            editor.addListener( 'selectionchange', function ( t, causeByUi ) {
                ...
                // }
                  if ( img != null && img.tagName.toLowerCase() == 'img' ) {
                    if ( img.getAttribute( 'class' ) == 'edui-faked-video' ) {
                        //锚点处理
                        html += popup.formatHtml(
                            '<nobr>视频: <a target="_blank" href="' + img.getAttribute( 'data-video-url' ) + '" >' + img.getAttribute( 'data-video-title' )) + '</a>' ;
                    }
                }
                var link;
                if ( editor.selection.getRange().collapsed ) {
                                        //按下标题
                    var node = domUtils.findParent(editor.selection.getStart(), function(node) {
                                            return node.tagName.toLowerCase() == 'h3'
                                        }, true);
                    ...
                    if(node){
                        ...
                        $("#" + editor.ui.toolbars[0].items[0].id).removeClass("edui-toolbar-menu-checked")
                    }
