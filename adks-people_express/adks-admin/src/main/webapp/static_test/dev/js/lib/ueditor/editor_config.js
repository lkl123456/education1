...
(function () {
    ...
    var URL= 'http://' + window.location.host + '/ueditor2/';

    UEDITOR_CONFIG = {
        imagePath:'http://' + window.location.host, //图片文件夹所在的路径，用于显示时修正后台返回的图片url！具体图片保存路径需要在后台设置。！important
        ...
        toolbars:[
            ['Bold','Indent', '|',  'Print','InsertVideo', 'Link']
        ],
        //当鼠标放在工具栏上时显示的tooltip提示
        labelMap:{
            'anchor':'锚点', 'undo':'撤销', 'redo':'重做', 'bold':'插入标题', 'indent':'自动排版',
            'italic':'斜体', 'underline':'下划线', 'strikethrough':'删除线', 'subscript':'下标',
            'superscript':'上标', 'formatmatch':'格式刷', 'source':'源代码', 'blockquote':'引用',
            'pasteplain':'纯文本粘贴模式', 'selectall':'全选', 'print':'插入图片', 'preview':'预览',
            ...
            'fontfamily':'字体', 'fontsize':'字号', 'paragraph':'段落格式', 'insertimage':'图片', 'inserttable':'表格', 'link':'插入链接',
        ...
        'body{padding:0px;font-family:"宋体";font-size:14px;overflow:hidden;margin:8px 8px 0px 8px;line-height:1.4}' +
        //针对li的处理
        'li{clear:both}' +
        //设置段落间距
        'p{margin:0px 0px 0px 0px;overflow:hidden;font-size:14px;font-family:helvetica,"Helvetica Neue",arial,"liberation sans",freesans,sans-serif;line-height:21px;}' +
        //设置图片大小
        'body img,body h3 img,body p img{overflow:hidden;width:100%;_width:expression(document.body.clientWidth > 100% ? "100%" : "auto");}' +
        //设置标题文字大小
        'body h3{overflow:hidden;clear:both;display:block;font-size:18px;padding:0px 0px 5px 0px;margin:10px 0px 10px 0px}' +
        //清除表格浮动状态
        '.tableclear{clear:both;margin:0;padding:0;}',
        //初始化编辑器的内容,也可以通过textarea/script给值，看官网例子
        initialContent:'',
        autoClearinitialContent:false, //是否自动清除编辑器初始内容，注意：如果focus属性设置为true,这个也为真，那么编辑器一上来就会触发导致初始化的内容看不到了
        //iframeCssUrl:'themes/default/uuu.css', //要引入css的url
...
//            var block = X(inline, {p:1,div:1,blockquote:1,$:{style:1,dir:1}});
            return {
                ...
                blackList:{style:1, font:1, span:1, script:1, link:1, object:1, applet:1, div:1, input:1, meta:1, base:1, button:1, select:1, textarea:1, '#comment':1, 'map':1, 'area':1}
//                whiteList: {
